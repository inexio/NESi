...
def show_onts(box_id):
    ...
    response = show_components(OntsSchema(), 'onts', req, box_id)
    return response, 200


@app.route(PREFIX + '/boxen/<box_id>/onts/<id>', methods=['GET'])
def show_ont(box_id, id):
    response = show_component('onts', box_id, id)
    return response, 200
...
def update_ont(box_id, id):
    req = flask.request.json
    update_component('onts', req, box_id, id)
