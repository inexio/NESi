...
def show_vlans(box_id):
    ...
    response = show_components(VlansSchema(), 'vlans', req, box_id)
    return response, 200


@app.route(PREFIX + '/boxen/<box_id>/vlans/<id>', methods=['GET'])
def show_vlan(box_id, id):
    response = show_component('vlans', box_id, id)
    return response, 200
